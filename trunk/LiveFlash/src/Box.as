...
class Box
{
	...
	public function Box(mc:MovieClip,server:LiveSocket)
	{
		handle = mc.handle;
		socket = server;
	...
	{
		handle.startDrag(true,0,0,200,200);
		handle.onEnterFrame = Delegate.create(this,updatePos);
	}
	
	private function updatePos()
	{
		var x:Number = (handle._x / 200);
		var y:Number = (handle._y / 200);
		
		if(!socket.isWaiting())
		{
			socket.sendCommand("blerp 0 0 1 2 3 " + x + " " + y);
		}
	}
